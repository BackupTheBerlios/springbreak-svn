...
// $Id: RSS_1_0_Parser.java,v 1.39 2003/11/04 19:09:43 niko_schmuck Exp $
...
/**
 * Parser which reads in document instances according to the RSS 1.0 (RDF)
 * specification and generates a news channel object.
 * 
 * @author Niko Schmuck
 */
class RSS_1_0_Parser implements RSS_1_0_ParserIF {
	...
	public ChannelIF parse(ChannelBuilderIF cBuilder, Element root)
		...
		// link element
		chnl.setSite(ParserUtils
				.getURL(channel.getChildTextTrim("link", defNS)));
		...
		if (publisher == null) {
			Element elErrorReportsTo = channel.getChild("errorReportsTo",
					adminNS);
		...
		if (updatePeriod != null) {
			chnl.setUpdatePeriod(updatePeriod.getTextTrim());
		}
		...
		if (updateFrequency != null) {
			chnl
					.setUpdateFrequency((new Integer(updateFrequency
		...
		while (i.hasNext()) {
			 ...
			 * 
			 * However it's part of the Spec and will eventually used at a later
			 * point in our application
			 * 
			 * Roland Vecera 10. Feb 2005
			 *  // get source element - default to Aggregation module, then try
			 * Dublin Core String sourceName = null; String sourceLocation =
			 * null; Date sourceTimestamp = null;
			 * 
			 * Element elSourceURL = item.getChild("sourceURL", agNS); if
			 * (elSourceURL == null) { // No Aggregation module - try Dublin
			 * Core elSourceURL = item.getChild("source", dcNS); if (elSourceURL !=
			 * null) { sourceLocation = elSourceURL.getTextTrim(); sourceName =
			 * "Source"; } } else { // Aggregation module sourceLocation =
			 * elSourceURL.getTextTrim(); Element elSourceName =
			 * item.getChild("source", agNS); if (elSourceName != null) {
			 * sourceName = elSourceName.getTextTrim(); } Element
			 * elSourceTimestamp = item.getChild("timestamp", agNS); if
			 * (elSourceTimestamp != null) { sourceTimestamp =
			 * ParserUtils.getDate(elSourceTimestamp.getTextTrim()); } }
			 * 
			 * if (sourceLocation != null) { ItemSourceIF itemSource =
			 * cBuilder.createItemSource(rssItem, sourceName, sourceLocation,
			 * sourceTimestamp); rssItem.setSource(itemSource); }
			 */
		...
		if (image != null) {
			ImageIF rssImage = cBuilder.createImage(image.getChildTextTrim(
					"title", defNS), ParserUtils.getURL(image.getChildTextTrim(
					"url", defNS)), ParserUtils.getURL(image.getChildTextTrim(
					"link", defNS)));
		 ...
		 * 
		 * Roland Vecera 10. Feb 2005
		 *  // textinput element Element txtinp = root.getChild("textinput",
		 * defNS); if (txtinp != null) { String tiTitle = null; if
		 * (txtinp.getChild("title", defNS) != null) { tiTitle =
		 * txtinp.getChild("title", defNS).getTextTrim(); } String tiDescr =
		 * null; if (txtinp.getChild("description", defNS) != null) { tiDescr =
		 * txtinp.getChild("description", defNS).getTextTrim(); } String tiName =
		 * null; if (txtinp.getChild("name", defNS) != null) { tiName =
		 * txtinp.getChild("name", defNS).getTextTrim(); } URL tiLink = null; if
		 * (txtinp.getChild("link", defNS) != null) { tiLink =
		 * ParserUtils.getURL(txtinp.getChild("link", defNS).getTextTrim()); }
		 * TextInputIF rssTextInput = cBuilder.createTextInput(tiTitle, tiDescr,
		 * tiName, tiLink); chnl.setTextInput(rssTextInput); }
		 */
	 ...
	 */
	private static int getTtl(final String updatePeriod, int updateFrequency) {
		int minutes = -1;
		if (updatePeriod.equals(ChannelIF.UPDATE_HOURLY)) {
			minutes = 60;
		} else if (updatePeriod.equals(ChannelIF.UPDATE_DAILY)) {
			minutes = 24 * 60;
		} else if (updatePeriod.equals(ChannelIF.UPDATE_WEEKLY)) {
			minutes = 168 * 60;
		} else if (updatePeriod.equals(ChannelIF.UPDATE_MONTHLY)) {
			minutes = 30 * 24 * 60;
		} else if (updatePeriod.equals(ChannelIF.UPDATE_YEARLY)) {
			minutes = 365 * 24 * 60;
		} else {
			minutes = 24 * 60;
		}
		return (minutes / updateFrequency);
	}
}